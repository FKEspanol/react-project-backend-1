...
const validateRequestBody = (request) => {
  ...
  schema.map((i) => {
    if (!i.value) {
      errors.push({
        errorMessage: `${i.name} is required !`,
        key: i.propName,
      });
    }
    ...
    if (
      (/[\W|\d|\s]/.test(i.value) && i.propName === "firstname") ||
      (/[\W|\d|\s]/.test(i.value) && i.propName === "lastname")
    ) {
      errors.push({
        errorMessage: `${i.name} name cannot contain numbers and special characters !`,
        key: i.propName,
      });
    }
